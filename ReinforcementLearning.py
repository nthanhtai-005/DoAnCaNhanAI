...
def q_learning_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "Q-Learning start")
    validate_state(goal, "Q-Learning goal")

    # Tham số Q-Learning
    alpha = 0.1  # Tỷ lệ học
    gamma = 0.9  # Yếu tố giảm giá
    epsilon = 0.1  # Tham số epsilon-greedy
    max_episodes = 1000  # Số tập huấn luyện tối đa
    max_steps = 100  # Số bước tối đa mỗi tập

    # Khởi tạo bảng Q
    q_table = {}
    visited = set()
    states_visited = 0
    steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]  # Các hướng di chuyển

    print("Q-Learning: Bắt đầu huấn luyện")
    print(f"Trạng thái ban đầu:\n{start}")
    print(f"Trạng thái mục tiêu:\n{goal}")

    def get_actions(state):
        try:
            x, y = location(state)
            actions = [(x + dx, y + dy) for dx, dy in steps if 0 <= x + dx <= 2 and 0 <= y + dy <= 2]
            print(f"Q-Learning: Các hành động khả thi tại trạng thái:\n{state}\nHành động: {actions}")
            return actions
        except ValueError as e:
            print(f"Q-Learning: Lỗi khi lấy hành động - {str(e)}")
            raise ValueError(f"Trạng thái không hợp lệ trong Q-Learning: {str(e)}")

    def choose_action(state, actions):
        state_tuple = tuple(state.flatten())
        if state_tuple not in q_table:
            q_table[state_tuple] = {action: 0.0 for action in actions}
            print(f"Q-Learning: Khởi tạo Q-values cho trạng thái {state_tuple}: {q_table[state_tuple]}")
        if random.random() < epsilon:
            action = random.choice(actions)
            print(f"Q-Learning: Chọn hành động ngẫu nhiên (epsilon): {action}")
        else:
            q_values = q_table[state_tuple]
            action = max(q_values, key=q_values.get)
            print(f"Q-Learning: Chọn hành động tối ưu: {action}, Q-values: {q_values}")
        return action

    def get_reward(state, next_state, goal):
        if np.array_equal(next_state, goal):
            reward = 100
            print(f"Q-Learning: Đạt mục tiêu, phần thưởng: {reward}")
        else:
            current_h = heuristic(state, goal)
            next_h = heuristic(next_state, goal)
            reward = -1 + (current_h - next_h) * 2
            print(f"Q-Learning: Phần thưởng: {reward}, Heuristic hiện tại: {current_h}, Heuristic tiếp theo: {next_h}")
        return reward
    ...
    for episode in range(max_episodes):
        if stop_flag or states_visited >= MAX_STATES_VISITED or time.time() - start_time >= MAX_TIME:
            end_time = time.time()
            print(f"Q-Learning: Dừng huấn luyện sau {episode} tập, "
                  f"Trạng thái đã thăm: {states_visited}, Thời gian: {end_time - start_time:.2f}s")
            print(f"Q-Learning: Lý do dừng - Stop flag: {stop_flag}, "
                  f"Max states: {states_visited >= MAX_STATES_VISITED}, Max time: {time.time() - start_time >= MAX_TIME}")
            break

        print(f"Q-Learning: Bắt đầu tập {episode + 1}")
        current_state = start.copy()
        for step in range(max_steps):
            actions = get_actions(current_state)
            if not actions:
                print("Q-Learning: Không có hành động khả thi, thoát tập")
                break
            ...
            reward = get_reward(current_state, next_state, goal)
            next_actions = get_actions(next_state)
            if next_state_tuple not in q_table:
                q_table[next_state_tuple] = {a: 0.0 for a in next_actions}
                print(f"Q-Learning: Khởi tạo Q-values cho trạng thái mới {next_state_tuple}")

            # Cập nhật Q-value
            current_q = q_table[state_tuple][action]
            future_q = max(q_table[next_state_tuple].values()) if next_actions else 0
            q_table[state_tuple][action] = current_q + alpha * (reward + gamma * future_q - current_q)
            print(f"Q-Learning: Cập nhật Q-value cho trạng thái {state_tuple}, hành động {action}: {q_table[state_tuple][action]}")

            current_state = next_state
            if np.array_equal(current_state, goal):
                print(f"Q-Learning: Đạt trạng thái mục tiêu trong tập {episode + 1}, bước {step + 1}")
                break

    # Suy ra đường đi tối ưu
    print("Q-Learning: Bắt đầu suy ra đường đi tối ưu")
    path = []
    current_state = start.copy()
    max_steps = 200  # Tăng giới hạn bước
    path_visited = set()  # Tập riêng để theo dõi trạng thái trong đường đi
    path_visited.add(tuple(current_state.flatten()))

    while not np.array_equal(current_state, goal) and max_steps > 0:
        if stop_flag or states_visited >= MAX_STATES_VISITED or time.time() - start_time >= MAX_TIME:
            end_time = time.time()
            print(f"Q-Learning: Dừng suy ra đường đi, "
                  f"Trạng thái đã thăm: {states_visited}, Thời gian: {end_time - start_time:.2f}s")
            print(f"Q-Learning: Lý do dừng - Stop flag: {stop_flag}, "
                  f"Max states: {states_visited >= MAX_STATES_VISITED}, Max time: {time.time() - start_time >= MAX_TIME}")
        ...
        actions = get_actions(current_state)
        if not actions:
            print("Q-Learning: Không có hành động khả thi trong suy ra đường đi")
            break
        state_tuple = tuple(current_state.flatten())
        if state_tuple not in q_table:
            print(f"Q-Learning: Trạng thái {state_tuple} không có trong q_table, dừng suy ra")
            break
        ...
        next_state_tuple = tuple(next_state.flatten())
        if next_state_tuple in path_visited:  # Kiểm tra vòng lặp trong đường đi hiện tại
            print(f"Q-Learning: Phát hiện vòng lặp trong đường đi tại trạng thái {next_state_tuple}, dừng suy ra")
            break
        ...
        current_state = next_state
        print(f"Q-Learning: Thêm bước vào đường đi: {action}, Trạng thái mới:\n{current_state}")
        max_steps -= 1

    end_time = time.time()
    if np.array_equal(current_state, goal):
        print(f"Q-Learning: Tìm thấy đường đi với {len(path)} bước, "
              f"Trạng thái đã thăm: {states_visited}, Thời gian: {end_time - start_time:.2f}s")
        return path, states_visited, end_time - start_time, visited
    print(f"Q-Learning: Không tìm thấy đường đi, "
          f"Trạng thái đã thăm: {states_visited}, Thời gian: {end_time - start_time:.2f}s")
    return None, states_visited, end_time - start_time, visited
