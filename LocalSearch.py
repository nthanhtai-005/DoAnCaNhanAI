...
class AlgorithmInfoFrame(tk.Frame):
    ...
    def update_info(self, algorithm=None, steps=None, states=None, runtime=None):
        ...
        if runtime is not None:
            self.thời_gian_chạy_value.config(text=f"{runtime:.4f} giây")
...
class ControlPanel(tk.Frame):
    def __init__(self, parent, start_callback, stop_callback, reset_callback, **kwargs):
        ...
        self.selected_algorithm = "Simple Hill Climbing"
...
class PuzzleSolverApp:
    ...
    def run_algorithm(self, algorithm_name):
        ...
        algorithm_map = {
            ...
            "Simulated Annealing": simulated_annealing_process,
            "Beam Search": beam_search_process,
            "Genetic Algorithm": genetic_algorithm_process,
        }
...
def heuristic(state, goal):
    ...
    for i in range(3):
        for j in range(3):
            if state[i, j] != 0:
                goal_pos = GOAL_POSITIONS[state[i, j]]
    ...
    return manhattan_distance + 2 * misplaced_tiles
...
def simple_hill_climbing(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "Simple HC start")
    validate_state(goal, "Simple HC goal")
    ...
    while not np.array_equal(current_state, goal) and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        try:
            x, y = location(current_state)
        except ValueError as e:
            raise ValueError(f"Trạng thái không hợp lệ trong Simple HC: {str(e)}")
        best_state = current_state
        best_heuristic = heuristic(current_state, goal)
        best_move = None
        ...
        if np.array_equal(best_state, current_state):
            end_time = time.time()
...
def steepest_ascent_hill_climbing(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "Steepest HC start")
    validate_state(goal, "Steepest HC goal")
    current_state = start.copy()
    path = []
    visited = set()
    visited.add(tuple(start.flatten()))
    ...
    while not np.array_equal(current_state, goal) and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong Steepest HC: {str(e)}")
        ...
        for dx, dy in steps:
            ...
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                ...
                if state_tuple not in visited:
                    visited.add(state_tuple)
                    states_visited += 1
                    neighbors.append((heuristic(new_state, goal), new_state, (new_x, new_y)))
        ...
        if best_heuristic >= heuristic(current_state, goal):
            end_time = time.time()
            return None, states_visited, end_time - start_time, visited
...
def stochastic_hill_climbing(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "Stochastic HC start")
    validate_state(goal, "Stochastic HC goal")
    current_state = start.copy()
    path = []
    visited = set()
    visited.add(tuple(start.flatten()))
    ...
    while not np.array_equal(current_state, goal) and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong Stochastic HC: {str(e)}")
        neighbors = []
        ...
        for dx, dy in steps:
            ...
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                ...
                if state_tuple not in visited:
                    ...
                    states_visited += 1
                    h = heuristic(new_state, goal)
                    neighbors.append((h, new_state, (new_x, new_y)))

        if not neighbors:
            end_time = time.time()
            return None, states_visited, end_time - start_time, visited

        current_h = heuristic(current_state, goal)
        better_neighbors = [(h, state, move) for h, state, move in neighbors if h <= current_h]
        if better_neighbors:
            _, next_state, move = random.choice(better_neighbors)
        else:
            _, next_state, move = random.choice(neighbors)
...
def simulated_annealing_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "SA start")
    validate_state(goal, "SA goal")
    
    if not is_solvable(start, goal):
        return None, 0, 0, set()
    
    current_state = start.copy()
    path = [current_state.copy()]  # Store full states
    visited = set()
    visited.add(tuple(current_state.flatten()))
    states_visited = 0
    
    temperature = 1000.0  # From provided code
    cooling_rate = 0.995  # From provided code
    min_temperature = 0.01  # From provided code
    
    while not np.array_equal(current_state, goal) and temperature > min_temperature and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        try:
            x, y = location(current_state)
        except ValueError as e:
            raise ValueError(f"Trạng thái không hợp lệ trong Simulated Annealing: {str(e)}")
        ...
        next_state = swap(current_state, x, y, new_x, new_y)
    ...
    # Convert state path to move path for GUI compatibility
    if np.array_equal(current_state, goal):
        move_path = []
        for i in range(len(path) - 1):
            try:
                x1, y1 = location(path[i])
                x2, y2 = location(path[i + 1])
                if abs(x1 - x2) + abs(y1 - y2) == 1:  # Ensure valid move
                    move_path.append((x2, y2))
                else:
                    return None, states_visited, time.time() - start_time, visited
            except ValueError:
                return None, states_visited, time.time() - start_time, visited
        end_time = time.time()
        return move_path, states_visited, end_time - start_time, visited
    
    end_time = time.time()
    return None, states_visited, end_time - start_time, visited

def beam_search_process(start, goal, stop_flag, beam_width=3):
    start_time = time.time()
    validate_state(start, "Beam Search start")
    validate_state(goal, "Beam Search goal")
    counter = 0
    beam = [(heuristic(start, goal), counter, start.copy(), [])]
    visited = set()
    visited.add(tuple(start.flatten()))
    steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    states_visited = 0

    while beam and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        new_beam = []
        for h, _, state, path in beam:
            validate_state(state, "Beam Search loop")
            ...
            try:
                ...
                raise ValueError(f"Trạng thái không hợp lệ trong Beam Search: {str(e)}")
            for dx, dy in steps:
                new_x, new_y = x + dx, y + dy
                if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                    ...
                    if state_tuple not in visited:
                        ...
                        states_visited += 1
                        counter += 1
                        new_beam.append((heuristic(new_state, goal), counter, new_state, path + [(new_x, new_y)]))
...
def genetic_algorithm_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "GA start")
    validate_state(goal, "GA goal")
    ...
    def is_solvable(state):
        """Kiểm tra tính khả giải dựa trên số hoán vị"""
        inv_count = 0
        flat = state.flatten()
    ...
    def fitness(state):
        """Hàm đánh giá: kết hợp Manhattan distance và ô đúng vị trí"""
        h = sum(abs(np.where(state == num)[0][0] - np.where(goal == num)[0][0]) +
                abs(np.where(state == num)[1][0] - np.where(goal == num)[1][0])
                for num in range(1, 9))
        correct_tiles = sum(1 for i in range(3) for j in range(3) if state[i, j] == goal[i, j])
        return -(2 * h + 0.5 * correct_tiles)  
    ...
    def generate_individual():
        ...
        while True:
            ...
            state = flat.reshape(3, 3)
            if is_solvable(state):
                validate_state(state, "GA generate_individual")
                return state

    def crossover(parent1, parent2):
        """Lai ghép: kết hợp trạng thái cha mẹ và sửa để khả giải"""
        flat1 = parent1.flatten()
        flat2 = parent2.flatten()
        cut = random.randint(1, 7)
        child_flat = np.concatenate((flat1[:cut], [x for x in flat2 if x not in flat1[:cut]]))
        child = child_flat.reshape(3, 3)
        if not is_solvable(child):
        ...
        validate_state(child, "GA crossover")
        return child

    def mutate(state):
        """Đột biến: hoán đổi ô trống với ô lân cận"""
        if random.random() < mutation_rate:
            x, y = location(state)
            steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]
            valid_steps = [(x + dx, y + dy) for dx, dy in steps if 0 <= x + dx <= 2 and 0 <= y + dy <= 2]
            if valid_steps:
                new_x, new_y = random.choice(valid_steps)
                state[x, y], state[new_x, new_y] = state[new_x, new_y], state[x, y]
        return state

    def reconstruct_path(start_state, goal_state):
        """Tái tạo đường đi từ start đến goal bằng BFS"""
        queue = deque([(start_state.copy(), [])])
        visited = set([tuple(start_state.flatten())])
        steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]
        while queue:
    ...
    if not is_solvable(start):
        end_time = time.time()
        return None, states_visited, end_time - start_time, visited
    population = [start.copy()] + [generate_individual() for _ in range(population_size - 1)]
    best_fitness = float('inf')
    generations_without_improvement = 0

    for gen in range(max_generations):
        if stop_flag or states_visited >= MAX_STATES_VISITED or time.time() - start_time >= MAX_TIME:
            end_time = time.time()
            return None, states_visited, end_time - start_time, visited
        population_fitness = [fitness(ind) for ind in population]
        sorted_indices = np.argsort(population_fitness)
        population = [population[i] for i in sorted_indices]
        population_fitness = [population_fitness[i] for i in sorted_indices]
        if np.array_equal(population[0], goal):
            end_time = time.time()
        ...
        while len(new_population) < population_size:
            ...
            parent1 = population[min(tournament, key=lambda i: population_fitness[i])]
            tournament = random.sample(range(len(population)), tournament_size)
            parent2 = population[min(tournament, key=lambda i: population_fitness[i])]
            child = crossover(parent1, parent2)
            child = mutate(child)
            new_population.append(child)
            state_tuple = tuple(child.flatten())
            if state_tuple not in visited:
                visited.add(state_tuple)
                states_visited += 1
