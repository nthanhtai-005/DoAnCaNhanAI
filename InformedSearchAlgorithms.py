...
class ControlPanel(tk.Frame):
    def __init__(self, parent, start_callback, stop_callback, reset_callback, **kwargs):
        ...
        self.selected_algorithm = "Greedy Best-First Search"
...
class PuzzleSolverApp:
    ...
    def run_algorithm(self, algorithm_name):
        ...
        algorithm_map = {
            "Greedy Best-First Search": greedy_process,
            "A* Search": a_star_process,
            "Iterative Deepening A* Search": ida_star_process
        }
...
def greedy_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "Greedy start")
    validate_state(goal, "Greedy goal")
    counter = 0
    heap = [(heuristic(start, goal), counter, start.copy(), [])]
    visited = set()
    ...
    while heap and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        ...
        validate_state(state, "Greedy loop")
        ...
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong Greedy: {str(e)}")
        for dx, dy in steps:
            ...
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                ...
                if state_tuple not in visited:
                    ...
                    states_visited += 1
                    h_score = heuristic(new_state, goal)
                    counter += 1
                    heapq.heappush(heap, (h_score, counter, new_state, path + [(new_x, new_y)]))
    end_time = time.time()
    return None, states_visited, end_time - start_time, visited

def a_star_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "A* start")
    validate_state(goal, "A* goal")
    counter = 0
    heap = [(heuristic(start, goal), 0, counter, start.copy(), [])]
    visited = set()
    visited.add(tuple(start.flatten()))
    steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    states_visited = 0
    ...
    while heap and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        f_score, g_score, _, state, path = heapq.heappop(heap)
        validate_state(state, "A* loop")
        if np.array_equal(state, goal):
            end_time = time.time()
            return path, states_visited, end_time - start_time, visited
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong A*: {str(e)}")
        for dx, dy in steps:
            new_x, new_y = x + dx, y + dy
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                new_state = swap(state, x, y, new_x, new_y)
                ...
                if state_tuple not in visited:
                    ...
                    counter += 1
                    heapq.heappush(heap, (new_f_score, new_g_score, counter, new_state, path + [(new_x, new_y)]))
    end_time = time.time()
    return None, states_visited, end_time - start_time, visited

def ida_star_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "IDA* start")
    validate_state(goal, "IDA* goal")
    ...
    def search(state, g_score, path, threshold, visited):
        ...
        validate_state(state, "IDA* search")
        f_score = g_score + heuristic(state, goal)
        ...
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong IDA*: {str(e)}")
        for dx, dy in steps:
            new_x, new_y = x + dx, y + dy
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                new_state = swap(state, x, y, new_x, new_y)
                ...
                if state_tuple not in visited:
                    visited.add(state_tuple)
                    states_visited += 1
                    new_threshold, result = search(new_state, g_score + 1, path + [(new_x, new_y)], threshold, visited)
                    if result is not None:
    ...
    visited = set()
    visited.add(tuple(start.flatten()))
    while not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        new_threshold, result = search(start, 0, [], threshold, visited)
        if result is not None:
            end_time = time.time()
