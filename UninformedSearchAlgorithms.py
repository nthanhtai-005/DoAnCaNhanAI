...
class ControlPanel(tk.Frame):
    def __init__(self, parent, start_callback, stop_callback, reset_callback, **kwargs):
        ...
        self.selected_algorithm = "Breadth-First Search"
...
class PuzzleSolverApp:
    ...
    def solve_puzzle(self, algorithm, speed):
        try:
            ...
            if solution:
                ...
                self.root.after(0, lambda: self.status_bar.update_status(
                    f"Không tìm thấy lời giải với {algorithm}. Thời gian: {runtime:.4f}s"))
    ...
    def run_algorithm(self, algorithm_name):
        ...
        algorithm_map = {
            "Breadth-First Search": bfs_process,
            "Depth-First Search": dfs_process,
            "Uniform Cost Search": ucs_process,
            "Iterative Deepening Search": ids_process
        }
...
def bfs_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "BFS start")
    validate_state(goal, "BFS goal")
    ...
    while queue and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        ...
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong BFS: {str(e)}")
        for dx, dy in steps:
...
def dfs_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "DFS start")
    validate_state(goal, "DFS goal")
    stack = [(start.copy(), [])]
    visited = set()
    visited.add(tuple(start.flatten()))
    steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    ...
    while stack and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        state, path = stack.pop()
        validate_state(state, "DFS loop")
        if np.array_equal(state, goal):
            end_time = time.time()
            return path, states_visited, end_time - start_time, visited
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong DFS: {str(e)}")
        for dx, dy in steps:
            new_x, new_y = x + dx, y + dy
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                new_state = swap(state, x, y, new_x, new_y)
                ...
                if state_tuple not in visited:
                    visited.add(state_tuple)
                    states_visited += 1
                    stack.append((new_state, path + [(new_x, new_y)]))
    end_time = time.time()
    return None, states_visited, end_time - start_time, visited

def ucs_process(start, goal, stop_flag):
    start_time = time.time()
    validate_state(start, "UCS start")
    validate_state(goal, "UCS goal")
    counter = 0
    heap = [(0, counter, start.copy(), [])]
    visited = set()
    visited.add(tuple(start.flatten()))
    steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    states_visited = 0

    while heap and not stop_flag and states_visited < MAX_STATES_VISITED and time.time() - start_time < MAX_TIME:
        cost, _, state, path = heapq.heappop(heap)
        validate_state(state, "UCS loop")
        if np.array_equal(state, goal):
            end_time = time.time()
            return path, states_visited, end_time - start_time, visited
        try:
            ...
            raise ValueError(f"Trạng thái không hợp lệ trong UCS: {str(e)}")
        for dx, dy in steps:
            new_x, new_y = x + dx, y + dy
            if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                new_state = swap(state, x, y, new_x, new_y)
                ...
                if state_tuple not in visited:
                    ...
                    new_cost = cost + 1
                    counter += 1
                    heapq.heappush(heap, (new_cost, counter, new_state, path + [(new_x, new_y)]))
    end_time = time.time()
    return None, states_visited, end_time - start_time, visited

def ids_process(start, goal, stop_flag):
    start_time = time.time()
    for depth in range(MAX_DEPTH):
        print(f"Depth: {depth}, State:\n{start}")
        visited = set()  
        path, states_visited, _ = dls(start, goal, depth, visited, 0, stop_flag)
        if path is not None:
            end_time = time.time()
            print(f"Tìm thấy lời giải ở độ sâu {depth} với {states_visited} trạng thái đã thăm.")
            return path, states_visited, round(end_time - start_time, 4)
        else:
            print("No solution found at this depth limit.")
    return None, states_visited, round(time.time() - start_time, 4)

def dls(state, goal, depth_limit, visited, states_visited, stop_flag):
    validate_state(state, "DLS start")
    validate_state(goal, "DLS goal")
    
    stack = [(state.copy(), [], 0)]  
    steps = [(-1, 0), (1, 0), (0, -1), (0, 1)]  
    local_visited = set()  
    local_visited.add(tuple(state.flatten())) 
    local_states = 0  

    while stack and not stop_flag and local_states < MAX_STATES_VISITED:
        state, path, depth = stack.pop()  
        print(f"Depth: {depth}, State: \n{state}")  
        print(f"Stack size after depth {depth}: {len(stack)}") 
        
        if np.array_equal(state, goal):  
            print("Goal state reached!")  
            return path, local_states, visited
        
        if depth <= depth_limit: 
            try:
                x, y = location(state)
            except ValueError as e:
                raise ValueError(f"Trạng thái không hợp lệ trong DLS: {str(e)}")
            for dx, dy in steps:
                new_x, new_y = x + dx, y + dy
                if 0 <= new_x <= 2 and 0 <= new_y <= 2:
                    new_state = swap(state, x, y, new_x, new_y)
                    state_tuple = tuple(new_state.flatten())
                    if state_tuple not in local_visited and state_tuple not in visited:
                        local_visited.add(state_tuple)
                        visited.add(state_tuple)
                        local_states += 1
                        stack.append((new_state, path + [(new_x, new_y)], depth + 1))  
                        print(f"State added to stack: \n{new_state}")  
        else:
            print(f"Depth {depth} exceeded, no more states to explore at this depth.")  

    print("No solution found at this depth limit.")  
    return None, local_states, visited
